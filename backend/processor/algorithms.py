...
def get_k_objects(k: int, fields: list[str], algorithm: str, aggr_func: str) -> tuple[int, list]:
    match algorithm:
        ...
        case 'treshold':
            return get_k_treshold(k, fields, get_aggr_func(aggr_func))

def get_k_treshold(k: int, fields: list[str], aggr_func: callable) -> tuple[int,list]:
    fieldsList = []
    fieldsIndexes = {}
    for field in fields:
...
def get_k_naive(k: int, fields: list[str], aggr_func: callable) -> tuple[int,list]:
